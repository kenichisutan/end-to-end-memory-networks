...
Early memory-augmented models such as memory networks \citep{weston2015memory} used discrete or heavily supervised components. MemN2N \citep{sukhbaatar2015end} keeps a continuous memory, reads it with softmax attention, and is trained end to end with ordinary backpropagation, which is closer to how we use neural networks today.

Attention over a set of vectors is the same broad idea as in sequence-to-sequence models with an encoder state, except here the attended positions are explicit memory slots filled with sentence encodings, and the model may take several hops before predicting an answer token.
...
When position encoding is on, each sentence is represented as a weighted sum of word embeddings, with weights given by the closed form in \citet{sukhbaatar2015end} so that word order matters. When it is off, the representation is a plain sum of embeddings (bag of words). The question uses the same encoding mode.
